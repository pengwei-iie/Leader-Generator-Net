...
from bart_util import NLIProcessor, adjust_learning_rate, accuracy, warmup_learning_rate, load_and_cache_examples, \
    save_model, AverageMeter, ProgressMeter
from bart_model import BartForCL, LinearClassifier, PairSupConBart
...
def parse_option():
    ...
    # ckpt
    parser.add_argument('--ckpt_bart', type=str, default='', help="path to pre-trained model")
...
def test(val_loader, model, classifier, args):
    ...
    np.save(os.path.join('/'.join(args.ckpt_bart.split('/')[:-1]), 'fc.npy'), np.array(res["fc"]))
    np.save(os.path.join('/'.join(args.ckpt_bart.split('/')[:-1]), 'label.npy'), np.array(res["label"]))
    return top.avg
...
def main():
    ...
    if args.seed is not None:
        ...
        warnings.warn(
            'You have chosen to seed training. This will turn on the CUDNN deterministic setting, which can slow down your training considerably! You may see unexpected behavior when restarting '
            'from checkpoints.')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    model = PairSupConBart(BartForCL.from_pretrained(
        "facebook/bart-base",  # Use the 12-layer BART model, with an uncased vocab.
    ...
    classifier = LinearClassifier(BartForCL.from_pretrained(
        "facebook/bart-base",  # Use the 12-layer BART model, with an uncased vocab.
    ...
    ckpt_bart = torch.load(args.ckpt_bart, map_location='cpu')
    ...
    model.load_state_dict(ckpt_bart['model'])
