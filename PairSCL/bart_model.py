...
from transformers.models.bart.modeling_bart import PretrainedBartModel, BartEncoder
from bart_util import masked_softmax, weighted_sum, sort_by_seq_lens


class BartForCL(PretrainedBartModel):
    def __init__(self, config):
        super(BartForCL, self).__init__(config)
        #xxx 768
    ...
    def forward(self, input_ids=None, attention_mask=None, head_mask=None, inputs_embeds=None, 
        ...
        sequence_output = outputs[1]  # pooler_output
        ...
        output = (logits,) + outputs[2:]  # past_key_values,hidden_states,attentions,cross_attentions
...
class PairSupConBart(nn.Module):
    def __init__(self, encoder, dropout=0.5, is_train=True):
        super(PairSupConBart, self).__init__()
        self.encoder = encoder.bart
        ...
        self.pooler = nn.Sequential(nn.Linear(self.dim_mlp, self.dim_mlp, bias=True),
                                    nn.Tanh())
    ...
    def forward(self, input_ids=None, attention_mask=None, head_mask=None, inputs_embeds=None):
        ...
        pair_output = self.pooler(self.fc(pair_embeds)[:, 0, :])
        if self.is_train:
