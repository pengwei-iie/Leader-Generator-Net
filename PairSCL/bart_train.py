...
from bart_util import NLIProcessor, adjust_learning_rate, warmup_learning_rate, load_and_cache_examples, save_model, AverageMeter, ProgressMeter
from bart_model import PairSupConBart, BartForCL
...
def parse_option():
    ...
    parser.add_argument('--dataset', type=str, default='SNLI',
                        choices=['SNLI', 'MNLI', 'SKILL', 'SKILL_Q'], help='dataset')
...
def train(train_loader, model, criterion_sup, criterion_ce, optimizer, epoch, args):
    ...
    for idx, batch in enumerate(train_loader):
        ...
        loss = criterion_sup(features, batch[2]) + criterion_ce(features, batch[2])
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    for epoch in range(args.start_epoch, args.epochs):  # epoch=10
        if args.distributed:
