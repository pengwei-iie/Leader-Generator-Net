...
def preprocess_SKILL_data(inputdir,
                         ...
                         eos=None):
    """
    Preprocess the data from the SKILL corpus so it can be used by the
    ESIM model.
    ...
    for file in os.listdir(inputdir):
        if fnmatch.fnmatch(file, "train_source"):
            train_file = file
        elif fnmatch.fnmatch(file, "val_source"):
            dev_file = file
        elif fnmatch.fnmatch(file, "test_source"):
            test_file = file
...
if __name__ == "__main__":
    default_config = "../config/skill_preprocessing.json"
