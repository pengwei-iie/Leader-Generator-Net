...
from data_processor_modify import Preprocessor


def preprocess_SKILL_data(inputdir,
                         targetdir,
                         lowercase=False,
    ...
    for file in os.listdir(inputdir):
        if fnmatch.fnmatch(file, "train.source"):
            train_file = file
        elif fnmatch.fnmatch(file, "val.source"):
            dev_file = file
        elif fnmatch.fnmatch(file, "test.source"):
            test_file = file
    ...
    print("\t* Reading data...")
    print(f"inputdir: {inputdir}, train_file: {train_file}")
    data = preprocessor.read_data(os.path.join(inputdir, train_file))
    data['hypotheses'] = []
    # {dict:4} ids=['xxx.jpg#4'] premises=[['A','person',...],...] hypotheses=[['A','person',...],...] labels=['neutral',...]
    print("\t* Computing worddict and saving it...")
    preprocessor.build_worddict(data)  # build vocab dict
...
if __name__ == "__main__":
    default_config = "../config/skill_Q_preprocessing.json"

    parser = argparse.ArgumentParser(description="Preprocess the SKILL dataset")
    parser.add_argument(
        "--config",
        default=default_config,
        help="Path to a configuration file for preprocessing SKILL"
    )
