...
def parse_option():
    ...
    parser.add_argument('--dataset', type=str, default='SNLI',
                        choices=['SNLI', 'MNLI', 'EXIM', 'SKILL'], help='dataset')
...
def main():
    ...
    if args.label_num == 3:
        ...
        label_map = ['action', 'causal relationship', 'character', 'feeling', 'outcome resolution', 'prediction', 'setting']
    else:
    ...
    print('ckpt_bert: ', args.ckpt_bert)
    print('ckpt_classifier: ', args.ckpt_classifier)
    ckpt_bert = torch.load(args.ckpt_bert, map_location='cpu')
    ckpt_classifier = torch.load(args.ckpt_classifier, map_location='cpu')
    if isinstance(ckpt_bert, torch.nn.DataParallel):
        ckpt_bert = ckpt_bert.module
    if isinstance(ckpt_classifier, torch.nn.DataParallel):
        ckpt_classifier = ckpt_classifier.module
    ...
    model.load_state_dict(ckpt_bert['model'], False)
    classifier.load_state_dict(ckpt_classifier['model'], False)
    ...
    if args.dataset in ['SNLI', 'EXIM', 'SKILL']:
