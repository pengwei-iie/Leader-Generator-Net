    ...
    def get_examples(self):
        ...
        for i, id in enumerate(self.data["ids"]):
            examples.append(InputExample(guid=str(id), text_a=' '.join(self.data["premises"][i]),
                                         text_b=' '.join(self.data["hypotheses"][i]) if self.data["hypotheses"] else '', label=self.data["labels"][i]))
        return examples
...
def load_and_cache_examples(args, processor, tokenizer, evaluate, dataset):
    ...
    if os.path.exists(cached_features_file):
        ...
        if evaluate == "test_match" or evaluate == "test_mismatch":
            ...
            dataset = TensorDataset(all_input_ids, all_attention_mask, all_labels, all_guid)
        else:
            # print(features[0])
            all_input_ids = torch.tensor([f.input_ids for f in features], dtype=torch.long)
            all_attention_mask = torch.tensor([f.attention_mask for f in features], dtype=torch.long)
        ...
        if evaluate == "test_match" or evaluate == "test_mismatch":
            ...
            examples = processor.get_examples()  # list:549367 guid='xxx.jpg#4' label='neutral' text_a='A person...' text_b='A person...'
            features = convert_examples_to_features(
                examples,
                tokenizer,
                max_length=args.max_seq_length,
            ...
            torch.save(features, cached_features_file)  # list:549367 attention_mask=[1,...] input_ids=[101,...] label=2 token_type_id=[0,...]
            all_input_ids = torch.tensor([f.input_ids for f in features], dtype=torch.long)
            all_attention_mask = torch.tensor([f.attention_mask for f in features], dtype=torch.long)
            all_labels = torch.tensor([f.label for f in features], dtype=torch.long)
            dataset = TensorDataset(all_input_ids, all_attention_mask, all_labels)
