...
def test(val_loader, model, classifier, args):
    ...
    with torch.no_grad():
        ...
        for idx, batch in enumerate(val_loader):
            ...
            res["input"] += batch[0].cpu().numpy().tolist()
            res["proj"] += features.cpu().numpy().tolist()
    ...
    np.save(os.path.join('/'.join(args.ckpt_bert.split('/')[:-1]), 'test_label.npy'), np.array(res["label"]))
    np.save(os.path.join('/'.join(args.ckpt_bert.split('/')[:-1]), 'test_input.npy'), np.array(res["input"]))
    np.save(os.path.join('/'.join(args.ckpt_bert.split('/')[:-1]), 'test_proj.npy'), np.array(res["proj"]))
    return top.avg
