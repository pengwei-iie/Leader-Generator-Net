...
from src.utils.lightning_base import BaseTransformer   # , add_generic_args, generic_train
from transformers import MBartTokenizer, T5ForConditionalGeneration
from transformers.modeling_bart import shift_tokens_right
from src.utils.callbacks import Seq2SeqLoggingCallback, get_checkpoint_callback, get_early_stopping_callback
...
DEFAULT_DEVICE = "cpu"  # "0" if torch.cuda.is_available() else "cpu"
conf = Config()
...
def generate_summaries_or_translations(examples: List[str], model_name: str, batch_size: int = 8,
...
# For QA Task
def e2eQA(source_dir, dataset_type):
    input_for_QA_model = []
    ...
    output_for_QA_model = run_generate(input_for_QA_model)  # ans
    ...
    f = open(os.path.join(source_dir, f'predict_{dataset_type}.txt'), 'w')
    for i in range(len(output_for_QA_model)):
...
if __name__ == "__main__":
    ...
    model: SummarizationModule = SummarizationModule(args)
    print(model)
    MODEL_PATH = os.path.join(conf.data_dir, 'save', 'p_e_1.ckpt')  # N
    ...
    bart_skill_dir = os.path.join(conf.data_dir, 'interim', 'BART_DATA', 'qa')  # BART_SKILL
    for m in ['val', 'test']:
        output_for_test = e2eQA(bart_skill_dir, m)  # question <SEP> text
        res, len_f = 0, 0
        with open(os.path.join(bart_skill_dir, f'{m}.target'), 'r', encoding='utf-8') as f:
