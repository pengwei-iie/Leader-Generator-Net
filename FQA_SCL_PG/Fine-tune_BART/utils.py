...
from typing import Callable, Dict, Iterable, List, Union, Tuple, Any
import sys
...
class AbstractSeq2SeqDataset(Dataset):
    def __init__(
        ...
        **kwargs
        ...
        self.scl_tokenizer = kwargs["scl_tokenizer"]
    ...
    def get_char_lens(data_file):
        return [len(x) for x in Path(data_file).open().readlines()]  # todo: remove exim part?
...
class Seq2SeqDataset(AbstractSeq2SeqDataset):
    ...
    def collate_fn(self, batch) -> Dict[str, Tensor]:
        ...
        src_texts, tgt_texts = [], []
        for x in batch:
            exim, question, context = x['src_texts'].split(" <SEP> ")
            src_texts.append(question + " <SEP> " + context)
            tgt_texts.append(x["tgt_texts"])
        ...
        batch_encoding["ids"] = torch.tensor([x["id"]])

        ## scl bert encoder related: get input_ids ##
        if self.scl_tokenizer is not None:
            scl_input = self.scl_data_fn(batch)
            batch_encoding["scl_input_ids"] = scl_input["input_ids"]
            batch_encoding["scl_attention_mask"] = scl_input["attention_mask"]
            batch_encoding["scl_token_type_ids"] = scl_input["token_type_ids"]
            batch_encoding["exim_ids"] = scl_input["exim_ids"]

        return batch_encoding

    def scl_data_fn(self, batch, max_seq_length=128):
        examples, exim_lst = [], []
        for x in batch:
            id = x["id"]
            exim, question, context = x['src_texts'].split(" <SEP> ")
            examples.append(InputExample(guid=str(id), text_a=question, text_b=context))
        ...
        # list:549367 attention_mask=[1,...] input_ids=[101,...] label=2 token_type_id=[0,...]
        all_input_ids = torch.tensor([f.input_ids for f in features], dtype=torch.long)
        all_attention_mask = torch.tensor([f.attention_mask for f in features], dtype=torch.long)
        all_token_type_ids = torch.tensor([f.token_type_ids for f in features], dtype=torch.long)
        all_exim_ids = torch.tensor(exim_lst, dtype=torch.long)

        return {"input_ids": all_input_ids, "attention_mask": all_attention_mask, "token_type_ids": all_token_type_ids,
                "exim_ids": all_exim_ids}
